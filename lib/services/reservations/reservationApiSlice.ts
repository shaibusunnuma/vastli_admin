...
import { BookingsResponse, ReservationStats, PERIOD, Reservation } from "@/types/reservations";
...
export const reservationApiSlice = createApi({
  ...
  endpoints: (build) => ({
    ...
    getReservationStats: build.query<ReservationStats, Partial<Reservation>>({
      query: (filter) => ({
        url: "/stats",
        params: filter,
      }),
